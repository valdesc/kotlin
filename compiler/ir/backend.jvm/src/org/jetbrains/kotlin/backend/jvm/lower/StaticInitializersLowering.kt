...
class StaticInitializersLowering(override val context: JvmBackendContext) : InitializersLoweringBase(context) {
    override fun lower(irClass: IrClass) {
        val staticInitializerStatements = extractInitializers(irClass) {
            ...
            (it is IrField && it.isStatic && it.constantValue(context) == null) || (it is IrAnonymousInitializer && it.isStatic) || (it is IrProperty && it.backingField?.isStatic == true && it.backingField!!.constantValue(context) == null)
        }.toMutableList()
